...
fn write_footer(
    ...
    let source_name = if original_link.contains("exhentai.org") {
        "ᴇxʜᴇɴᴛᴀɪ"
    } else if original_link.contains("e-hentai.org") {
        "ᴇ-ʜᴇɴᴛᴀɪ"
    } else if original_link.contains("nhentai") {
        "ɴʜᴇɴᴛᴀɪ"
    } else {
        "ᴜɴᴋɴᴏᴡɴ"
    };
    ...
    // 2. 動態計算並構建頁碼字串
    // 實際最後一張圖片的頁碼 = 起始頁碼 + 下載總數 - 1
    let real_end = start_idx + total_images.saturating_sub(1);
    
    let pages_str = if start_idx == 1 && end_idx.is_none() {
        format!("{}", total_images)           // 全量下載時，直接顯示總頁數 (如: 29)
    } else if start_idx == real_end {
        format!("{}", start_idx)              // 若只是一張單圖 (如: 5)
    } else {
        format!("{}-{}", start_idx, real_end) // 片段下載時顯示範圍 (如: 5-15)
    };

    let text_pages = format!(" ┆ ᴘᴀɢᴇꜱ : {}", pages_str);

    // 3. 組合 DOM 節點：► (純文字) + 來源名稱 (帶超連結) + ┆ ᴘᴀɢᴇꜱ : XX (純文字)
    content.push(np!(
        nt!("► "),
        na!(@original_link, nt!(source_name)),
        nt!(text_pages)
    ));
}
